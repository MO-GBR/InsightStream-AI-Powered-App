...
const Navbar = () => {
    ...
    return (
        <header className={`flex items-center justify-between max-lg:flex-col p-2 px-10 ${
            scrollY > 50 ? 'bg-gray-900/80 backdrop-blur-sm border-b border-gray-700' : 'bg-transparent'
        }`}>
            <nav className='flex-center'>
                <div className='flex-center max-lg:mb-2'>
                    <Image src='/logo.svg' alt='logo' width={30} height={30} />
                    <span className='text-lg font-bold text-white font-mono'>InsightStream</span>
                </div>
                <ul className='flex items-center gap-6 ml-10 max-lg:hidden'>
                    {
